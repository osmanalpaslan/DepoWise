...
### ADR-059 — Admin-tanımlı makine şubesi + IP'den il (10.07.2026, KISMÎ — sunucu+web bitti, masaüstü sürüyor)
...
- **Karar (Adım 2 — masaüstü, SÜRÜYOR / yapılacak):** Masaüstü kayıt yanıtından makine şubesini önbelleğe alacak; **ana sayfada makine şubesini** gösterecek (login şubesi değil); **farklı-şube uyarısını** admin-atanmış makine şubesine bağlayacak; **internet yoksa** makinenin şubesine **otomatik giriş** (seçim yok), internet varsa web-benzeri kimlik; **kullanıcı veya makine şubesi yoksa** girişi engelleyecek ("kullanıcıya/makineye şube tanımlanmamış"). Bu değişiklik yeni masaüstü paketi (1.0.35) ile görünür olacak.
